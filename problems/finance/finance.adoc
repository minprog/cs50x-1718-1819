...
Before starting out, you might want to take a look at this short from Doug wherein HTTP and its methods are explained. Refer to http://flask.pocoo.org/docs/1.0/quickstart/#http-methods[this link] to see how you can use this in your code.
...
Make sure to clearly indicate in your code what you have implemented as your personal touch. This can be done by putting `PERSONAL TOUCH: <explanation>` in your comments.
...
[source]
----
check50 --local cs50/2018/spring/finance
----
...
Toward CS50 IDE's top-left corner, within its "file browser" (not within a terminal window), control-click or right-click your `finance` folder and then select *Download*.

You should find that your browser has downloaded `finance.zip`. If instead, you find a folder named `finance`, make a `.zip` from it and submit it, below.
