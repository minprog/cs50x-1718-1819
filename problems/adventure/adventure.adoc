...
It already contains two methods, but you have to implement them!
...
We call he `load_rooms` method in the `__init__` of the Adventure class. We use it there because everytime we create a game, we want it to at least have a list of rooms. Any data an Adventure class needs to function properly is initialized here.
...
Here you'll find your first TODO. It's up to you to add the connections to each individual room object.
...
Next up is the `move()` method in `Adventure`. Here you will use `is_connected()` method and the `current_room` attribute in `Adventure`.
We've initialized that attribute in the `__init__()` and set it to the room with id 1. Make sure to use and update it in `move()` to keep track of where our player is.
