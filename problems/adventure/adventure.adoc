...
Now, we can create a `current_room` attribute in `Adventure` to keep track of where our player is. Make sure to initialize that to the room with id 0 at the start of the game, and to update it in the `move()` method.
