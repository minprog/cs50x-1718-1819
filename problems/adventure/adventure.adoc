...
The Room class can now be freely used in `adventure.py`.
...
You'll be creating these objects in the `load_rooms` method, though for now it is little more than a TODO.

`Won` will let the game know when you've completed the game. You still have to let won know when the game is won however.

Moving around in the game is done via the `move` method. Here you'll make use of the rooms you've created earlier.

The `play` method contains the main loop that makes your game playable. We've already given you a headstart here, but you'll have to figure out how to best use your classes.
...
This is your room class. The blueprint for the room objects that your game will need to function properly.
It is still mostly empty, the implementation is all up to you!
...
Implement the `Room` class and `load_rooms` method so that each room from TinyRooms.txt is represented by a Room object. Note that for now you can leave out the routes, just start out with an id, name and description.
...
Now when you run the script you should be met with a welcome message and be prompted for a command. It's time to implement playability.
...
Neither the class `Item` or the method to load them have been implemented yet. So you'll have to create the files and methods yourself.
...
Notice how the command consists of two parts, instead of just the one for moving in a direction. You might want to either split the user input to recocgnize a `TAKE` or `DROP` command.
