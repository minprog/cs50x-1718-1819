...
You'll be creating these objects in the `loadRooms` function, but for now it is still little more than a TODO.
...
Only one condition can exist for movements and it is always an item. If the player has the required item, the conditional movement is always made; it is not an optional movement.
