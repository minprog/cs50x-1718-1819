...
==== Items
Items in Adventure have a name, description and location. To add these items to the game we'll once again have to first parse their data file. You can find them inside the `SmallObjects.txt` and `CrowtherObjects.txt`.
They look alot like rooms, but are just a bit simpeler. In `SmallObjects.txt` you'll find the following data:
...
This is why it would be a great idea use a class for items. Both the class `Item` and the function to load them (`loadObjects`) have been created already. It's up to you to implement them.
...
Both cases should be met by printin "`No such object.`" Resulting in the following gameplay:
----
  You are inside a building, a well house for a large spring.
  KEYS: a set of keys
  > TAKE KEYS
...
Any item the player picks up, they can also drop using the `DROP` command. Ownership then transfers from the player to the room he is currently in (no throwing items to other rooms!). Once again when a player were to drop an item not in his possession they are met with "`No such object.`". Dropping items looks alot like picking them up:
----
  You are inside a building, a well house for a large spring.
  KEYS: a set of keys
  > TAKE KEYS
...
==== You shall not pass.
Sometimes a player will attempt a movement he cannot make yet, because they are missing the required item. Passing the steel grate in room 6 for example requires keys.
