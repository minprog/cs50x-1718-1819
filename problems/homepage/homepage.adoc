...
```
+
in the terminal window and clicking on the link that appears, which should be of the form `http://ide50-USERNAME.legacy.cs50.io:8080`, substituting, of course, your actual username on CS50 IDE in place of `USERNAME`.
...
```
http://ide50-USERNAME.legacy.cs50.io:XXXX
```
...
Follow these steps to create a zip file that you can then submit:

1. `cd ..` to the directory _containing_ the `pset5` directory
2. enter `zip -r pset5 pset5` to compress the complete pset5 folder into a zipfile
3. download the zipfile by right-clicking in the IDE's file browser and clicking "Download"

Your web browser may immediately decompress (unzip) the file after downloading. Unfortunately you can't submit it that way. Ask for help if needed!

Note: below, you will need to upload the zip _and also_ your html and css files.
