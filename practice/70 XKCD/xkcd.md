...
Create a website that looks up XKCD comics based on user input. This exercise focusses on learning Flask templates.
...
> Before you start implementing, write down on a piece of paper which parts of code that should be changed to implement this functionality.

### Step 1

Change the Flask code such that it accepts a list of comma-seperated numbers. Tip: have a look at the python function `split()`: [https://docs.python.org/2/library/stdtypes.html#str.split](https://docs.python.org/2/library/stdtypes.html#str.split).
...
Change the `index.html` template such that it first diplays only the first element of the comic list (similar to what you did in Exercsie 1).
...
Change the `index.html` such that it shows not only the first, but all the comics in the list.
...
It contains more information than only the comic URL and `alt` tag. Think what information you'd find interesting to know about the comic, an display that along with the image.